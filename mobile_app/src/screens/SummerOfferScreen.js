...
const ProductCard = ({ product, onAdd, onRemove, navigation, canAddToCart, getPromotionalPrice }) => {
  ...
  const renderSummerBadge = () => {
    ...
    switch (product.promotion.type) {
      case 'bundle':
        badgeText = `${product.promotion.quantityRequired}+${product.promotion.quantityFree} FREE`;
        break;
      case '2+1':
        badgeText = '2+1 OFFER';
        break;
    ...
    return (
      <View style={[styles.summerBadge, { backgroundColor: badgeColor }]}>
        <Text style={styles.summerBadgeText}>{badgeText}</Text>
      </View>
    );
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.priceContainer}>
        ...
        {product.promotion && product.promotion.type === 'bundle' && (
          <Text style={styles.discountedPrice}>
            {product.promotion.quantityRequired}+{product.promotion.quantityFree} FREE
          </Text>
        )}
      </View>
      {product.inCart > 0 ? (
...
const SummerOfferScreen = ({ navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      if (productsResult.success) {
        ...
        // Filter for summer-related products (those with promotions that might be summer-related)
        const summerProducts = allProducts.filter(product => 
          product.promotion && (
            product.promotion.title?.toLowerCase().includes('summer') ||
            product.promotion.type === 'bundle' || // Summer bundles
            product.promotion.type === '2+1' // Summer offers
          )
        );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Summer Offers</Text>
        <TouchableOpacity 
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            placeholder="Search summer offers..."
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading summer offers...</Text>
        </View>
      )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.noDataContainer}>
              <Text style={styles.noDataText}>No summer offers found</Text>
            </View>
          }
        />
      )}
...
export default SummerOfferScreen; 